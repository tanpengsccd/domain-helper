...
import {computed, h, onMounted, onBeforeUnmount, reactive, ref, createVNode, getCurrentInstance} from "vue";
import {
    DeleteOutlined,
    SyncOutlined,
    FormOutlined,
    ...
    UploadOutlined,
    DownloadOutlined, VerifiedOutlined, HistoryOutlined, CloudUploadOutlined
...
const refreshRecords = () => {
    ...
    loading.value = true
    dbsService.listRecords(nowDomainInfo.value.domain).then(r => {
        count.value = r.count
        //records.value = r.list

        // 获取当前域名下的所有ssl证书
        let allSsl = getAllSslInfo().filter(i => i.subdomain.includes(nowDomainInfo.value.domain))
        // 为 r.list 附加 ssl 证书信息 只有 A AAAA CNAME 类型的记录才有证书
        const now = Date.now();
        records.value = r.list.map(i => {
            let fulldomain = i.Name === "@" ? nowDomainInfo.value.domain : i.Name + "." + nowDomainInfo.value.domain
            if (["A", "AAAA", "CNAME"].includes(i.Type)) {
                i.ssl = allSsl.find(j => j.subdomain.includes(fulldomain))
                // 计算证书有效期
                if (i.ssl) {
                    i.ssl.expired = Math.floor((i.ssl.validTo - now) / 1000 / 60 / 60 / 24)
                }
                i.monitor = allMonitorRecords.value.find(j => j.uri === fulldomain)
                // 计算监控证书有效期
                if (i.monitor) {
                    i.monitor.expired = Math.floor((i.monitor.expire_time - now) / 1000 / 60 / 60 / 24)
                }
            }
            return i
        })
...
const deleteRecord = (record) => {
    ...
    Modal.confirm({
        ...
        content: h('div', null, [
            ...
            }, record.Name === "@" ? nowDomainInfo.value.domain : record.Name + "." + nowDomainInfo.value.domain),
        ]),
        okText: '确认',
        cancelText: '取消',
        onOk: deleteRecordDo,
...
const calcRecords = computed(() => {
    ...
    // 如果没有条件，直接返回所有记录
    if (!key && !searchForm.type && !searchForm.isMonitoring) {
        return records.value;
    }

    let filteredRecords = (records.value || []).filter(item => {
        const matchKeyword = (item.Name.toLowerCase().includes(key) || item.Value.toLowerCase().includes(key));
        const matchType = !searchForm.type || item.Type === searchForm.type;
        const matchMonitoring = !searchForm.isMonitoring || item.monitor;
        return matchKeyword && matchType && matchMonitoring;
    })
    if (!key) {
...
const addRecord = () => {
    if (addDomainRecordModal.value) {
        addDomainRecordModal.value.openModal(nowDomainInfo.value, null, records.value)
    }
}
...
const changeStatus = (value, e, record) => {
    Modal.confirm({
        ...
        content: h('div', null, [
            h('span', null, '确认修改记录'),
            h('span', {style: {color: colorPrimary.value, marginLeft: '10px'}}, record.Name),
            h('span', {style: {marginLeft: '10px'}}, '状态为'),
            h('span', {style: {color: value ? '#40c057' : '#f03e3e', marginLeft: '10px'}}, value ? '启用' : '暂停'),
        ]),
        okText: '确认',
        cancelText: '取消',
        onOk: () => {
            ...
            dnsService.changeRecordStatus(nowDomainInfo.value.domain, record.RecordId, {Status: value}).then(r => {
...
const editRecord = (record) => {
    if (addDomainRecordModal.value) {
        addDomainRecordModal.value.openModal(nowDomainInfo.value, record, records.value)
    }
}

const monitorRecordSsl = (record) => {
    const fullUrl = record.Name === "@" ? nowDomainInfo.value.domain : record.Name + "." + nowDomainInfo.value.domain
    addSslMonitor({
        uri: `https://${fullUrl}`,
        type: record.Type,
...
const rowSelection = {
    ...
    onChange: (keys, selectedRows) => {
        ...
        // 过滤 NS 和 SOA 类型 不能删除
        selectedRecords.value = selectedRows.filter(i => !['NS', 'SOA'].includes(i.Type))
    },
};
const batchDelete = () => {
    Modal.confirm({
        ...
        content: h('div', {style: {marginBottom: "10px",}}, [
            ...
            selectedRecords.value.map(i => h('div', {style: {color: colorPrimary.value}}, i.Name === "@" ? nowDomainInfo.value.domain : `${i.Name}.${nowDomainInfo.value.domain}`)),
...
const batchMonitor = () => {
    const canMonitor = selectedRecords.value.filter(i => ['A', 'CNAME', 'AAAA'].includes(i.Type))
    ...
    Modal.confirm({
        ...
        onOk: () => {
            ...
            batchAddSslMonitorLogic(canMonitor.map(i => ({
                uri: i.Name === "@" ? nowDomainInfo.value.domain : i.Name + "." + nowDomainInfo.value.domain,
                type: i.Type,
                address: i.Value,
                domain: nowDomainInfo.value.domain,
                remark: i.Remark,
    ...
    try {
        ...
        Modal.confirm({
            ...
            async onOk() {
                ...
                // 获取当前所有记录用于比对
                const currentRecords = await dnsService.listRecords(nowDomainInfo.value.domain);
                const results = [];
                ...
                for (const record of filteredRecords) {
                    try {
                        // 检查是否存在完全相同的记录
                        const exists = currentRecords.list.some(r =>
                            r.Name === record.Name &&
                            r.Type === record.Type &&
                ...
                Modal.info({
                    ...
                    content: h('div', null, [
                        h('div', {style: {marginBottom: '10px'}}, [
                            h('span', {style: {color: token.value.colorSuccess}}, `成功: ${successCount} 条`),
                            h('span', {
                                style: {
                                    marginLeft: '10px',
                        ...
                        h('div', {
                            ...
                            h('div', {
                                style: {
                                    marginBottom: '8px',
                                    color: status === 'error' ? token.value.colorError :
                                        status === 'skipped' ? token.value.colorWarning : token.value.colorSuccess
                                }
                            }, [
                                h('div', null, `${record.Name} | ${record.Type} | ${record.Value} | ${record.TTL}`),
...
<template>
    <div class="box_record">
        <a-flex class="h_header" align="center" gap="16px">
            <a-space :size="12">
                <div style="width: 100px;text-align: center">
                    <img :src="nowDomainInfo.cloud_info.icon" alt="" style="height: 17px;">
                </div>
                <a-select show-search :value="nowDomainInfo._id" @change="selectDomain" style="width: 240px;">
                    <a-select-option v-for="(i, index) in allDomains" :key="index" :value="i._id">{{
                            i.cloud_info.title
                        }}-{{ i.domain }}
                    </a-select-option>
                </a-select>
                <a-space :size="8" style="min-width: 200px;margin: 0 auto">
                    <a-tooltip>
                        ...
                        <a-space :size="4">
                            ...
                            <a-typography-text style="font-weight: 500;font-family: fantasy sans-serif;"
                    ...
                    <a-tooltip>
                        <a-space :size="4">
                            ...
                            <a-typography-text style="font-weight: 500;font-family: fantasy sans-serif;"
                                               :style="{color: token.colorPrimary}">{{ monitoringRecords.length }}
                            </a-typography-text>
                            <a-typography-text>条</a-typography-text>
                        </a-space>
                    </a-tooltip>
            ...
            <a-space :size="12">
                <a-button :icon="h(SyncOutlined)" @click="initRecords(nowDomainInfo)"></a-button>
        ...
        <a-flex class="header" :style="{backgroundColor:token.colorBgContainer}"
                justify="space-between">
            <a-flex align="baseline" gap="12">
                <a-select v-model:value="searchForm.type" allow-clear placeholder="解析类型" style="width: 160px;">
                    <a-select-option v-for="(i, index) in RecordTypes" :key="index" :value="i">{{ i }}</a-select-option>
                </a-select>
                <a-input-search v-model:value="searchForm.keyword" placeholder="输入关键字检索"
                                style="width: 200px;"></a-input-search>
            ...
            <a-space :size="12">
                ...
                <a-dropdown>
                    ...
                    <template #overlay>
                        <a-menu :disabled="selectedRecords.length === 0" @click="({ key }) => exportRecords(key)">
                ...
                <a-tooltip title="批量删除记录">
                    <a-button danger @click="batchDelete" :icon="h(DeleteOutlined)"
                              :disabled="selectedRecords.length === 0"></a-button>
                </a-tooltip>
                <a-tooltip title="批量监控SSL" placement="left">
                    <a-button @click="batchMonitor" :icon="h(MonitorOutlined)"
                              :disabled="selectedRecords.length === 0"></a-button>
                </a-tooltip>
            </a-space>
        </a-flex>
        <div class="body" ref="xbody">
            <a-table :row-selection="rowSelection" :locale="{emptyText: '暂无解析记录'}" sticky
                ...
                <template #bodyCell="{ column, record }">
                            ...
                            <a-space :size="4">
                                <a-tooltip v-if="record.ssl">
                                    <template #title>
                                        ...
                                        <div>
                                            {{
                                                record.ssl.expired > 0 ? `有效期 ${record.ssl.expired} 天` : "证书已过期"
                                            }}
                                        </div>
                                    </template>
                                    <SafetyCertificateOutlined
                    ...
                    <template v-if="column.key === 'Status'">
                        <template v-if="column.canEdit">
                            <a-switch @change="(k, e) => changeStatus(k, e, record, column)"
                                      v-model:checked="record.Status" un-checked-children="暂停"
                                      :checked-value="true" :un-checked-value="false"
                                      checked-children="启用"></a-switch>
                        </template>
                        <template v-else>
                    ...
                    <template v-if="column.key === 'ProxyStatus'">
                        <a-flex justify="center" align="center" v-if="record.ProxyStatus" :size="5">
                            ...
                            <template #overlay>
                                <a-menu @click="({ key }) => handleMenuClick(key, record)">
                                    <template v-if="['A', 'AAAA', 'CNAME'].includes(record.Type)">
                                        ...
                                        <a-menu-item key="pushSSL" v-if="record.ssl && record.ssl.expired > 5">
                                            <a-space size="small">
                                                <CloudUploadOutlined/>
                                                推送证书
                                            </a-space>
        ...
        <AddDomainRecord ref="addDomainRecordModal" @refresh="refreshRecords"></AddDomainRecord>
    </div>
</template>
